...
public:
  ...
  : Node("ur_mpc_node")
  {
    ...
    joint_cmd_sub_ = this->create_subscription<sensor_msgs::msg::JointState>("/joint_command", 10, std::bind(&UrMpcNode::trajectory_callback, this, std::placeholders::_1));
      ...
      urcl::vector6d_t joint_cmd;
      {
        ...
        if (trajectory_received_)
        {
          if (current_trajectory_index_ < trajectory_.points.size())
          {
            ...
            for (size_t i = 0; i < 6; ++i) {
              double vel = std::abs((pt.positions[i] - js.position[i]) / 0.008);
              if (vel > M_PI / 3) {
              position_command_[i] = js.position[i] + (pt.positions[i] > js.position[i] ? 1 : -1) * (M_PI / 3) * 0.008;
              } else {
              position_command_[i] = pt.positions[i];
              }
            }
          }
          else
